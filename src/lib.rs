...
impl FDNReverb {
    pub fn new(sample_rate: f32) -> FDNReverb {
        ...
        let allpass_frequency: Vec<f32> = delay_times
            .iter()
            .map(|frames| 1. / (3. * (*frames as f32) / sample_rate))
            .collect();
    ...
    pub fn process(&mut self, input: &[f32], output: &mut [f32]) {
        for (input, o) in input.iter().zip(output.iter_mut()) {
            ...
            for i in 0..4 {
                self.all_passes[i]
                    .process_single(*input + self.feedback[i], &mut all_passed_samples[i]);
            ...
            diffused_samples =
                matrix_vector_multiply(&delayed_samples, &self.feedback_matrix.into());
            for i in 0..4 {
